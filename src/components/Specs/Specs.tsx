...
export class Specs extends PureComponent<ISpecs, IState> {
  ...
  public UNSAFE_componentWillMount() {
    const showDescription =
      ...
      (this.props.description!.full.length > 0 ||
        this.props.description!.short.length > 0);
    const showSpecs =
      this.props.specs!.length > 0 &&
      this.props.specs![0].groupSpecs.length > 0;
    this.setState({
      showSpecs,
      showDescription
    });
  ...
  public render() {
    ...
    return (
      <div className={cnSpecs(null, { mode: mode! })}>
        {mode === "preview" && (showSpecs || showDescription)
          ? this.renderPreview()
          : null}
  ...
  private renderPreview = () => {
    ...
    const invisible = () => (
      <div className={cnSpecs("More", null, ["Specs-Content"])}>
        {showSpecs ? this.generateSpecs() : null}
      </div>
    );
    ...
    return (
      <div className={cnSpecs("Preview", null, ["Specs-Content"])}>
        ...
        {showDescription ? (
          <SpecsDescription text={this.props.description!.short} expandable />
        ) : null}
        {showSpecs ? (
          <Cut
            more="Посмотреть все характеристики"
            visible={visible()}
            invisible={invisible()}
          />
        ) : null}
  ...
  private generateItems = (groupIndex: number, limit: number = 0) => {
    ...
    return list.map(({ name, value }, index) => (
      <SpecsItem name={name} value={value} key={index} />
    ));
  };
