...
export const ScrollerContext = createContext<IScrollerContext>(
  {} as IScrollerContext
);
...
export class Scroller extends PureComponent<IScrollerProps, IScrollerState> {
  ...
  componentDidUpdate(
    _prevProps: IScrollerProps,
    _prevState: IScrollerState,
    snapshot: number | null
  ) {
    if (
      this.itemsLength !== this.items.length ||
      (this.domElem && this.domElem.scrollWidth !== snapshot)
    ) {
  ...
  protected scrollRight() {
    ...
    if (domElem) {
      if (rightPointScrollToEnd) {
        ...
        if (
          offset + wrapWidth >= rightPointScrollToEnd &&
          wrapWidth < rightPointScrollToEnd
        ) {
  ...
  protected checkEndOfList(domElem: HTMLElement) {
    if (
      !this.props.endEventOffset ||
      (!this.props.onScrollLeftEnd && !this.props.onScrollRightEnd)
    ) {
      return;
    }
    ...
    if (
      left > hidden - this.props.endEventOffset &&
