...
export class Amountpicker extends Component<IProps, IState> {
    ...
    public render(): ReactNode {
        ...
        const isDecDisabled = (this.state.value !== null && this.state.value <= MIN) || this.state.isInputFocused;
        const isIncDisabled = (this.state.value && this.state.value >= MAX) || this.state.isInputFocused;

        return (
            <div className={className ? cnAmountPicker(null, [className]) : cnAmountPicker()}>
                <Button
                    size="m"
                    theme="primary"
                    pin="circle-circle"
    ...
    private getEvent(valueData: { value: number | null; delta: number }): ICountEvent {
        return {
            target: {
